...
#Set the URL
#rdb stands for relational database. we're going to keep that as is.
theURLnew <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02085070&period=24&format=rdb'

#Get the data, which starts on line 30
gage_data <- read.table(theURLnew,skip = 30,header=TRUE,sep='\t',stringsAsFactors = T) 
...
the_states_tag <- 'tr :nth-child(1)'
the_price_tag <- 'tr :nth-child(2)'
the_capacity_tag <- 'tr :nth-child(3)'
net_generation_tag <- 'tr :nth-child(4)'
total_retail_tag <- 'tr :nth-child(5)'
  
#3 Construct a dataframe from the values
the_states <- the_website %>% html_nodes(the_states_tag) %>% html_text()
the_price <- the_website %>%   html_nodes(the_price_tag) %>%  html_text()
the_capacity <- the_website %>% html_nodes(the_capacity_tag) %>% html_text()
net_generation <- the_website %>% html_nodes(net_generation_tag) %>% html_text()
total_retail <- the_website %>% html_nodes(total_retail_tag) %>% html_text()

df_state_electricity <- data.frame(
  'state' = the_states,
  'price' = the_price,
  #find the way to do this in the powerpoint slides
...
df_years <- lapply(seq(2017,2022), scrape.it)
