...
# Rename columns
# c() is short for cbind which is short for column bind command
...
#shows all objects on data frame
str(USGS.flow.data)
#tells how many rows and columns you have on your data set
dim(USGS.flow.data)
...
> Answer: never remove NAs for a time series analysis because then you don't have a complete picture of your data. you can often omit NAs if they're all together at the beginning. if you want the mean and standard deviation then it might make sense to remove NAs because they could alter your data if left in.
...
# the % operator is usually used to format as a variable
...
#we are teaching R how to transform our current "dates" into actual dates
...
```{r Fixing dates}
# convert the date object back into a character object
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d")

# we are concatonating strings. gluing the 19 or 20 to the d input (whatever is assigned in the daytime column) depending on the result of the if statement
create.early.dates <- (function(d) {
       paste0(ifelse(d > 191226,"19","20"),d)
       })
# add the 19 to the initial observations
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)

# convert back to a date from a character object. using %Y because we are using a 4 digit year
