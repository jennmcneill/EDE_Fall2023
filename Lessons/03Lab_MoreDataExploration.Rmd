...
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #byrow/bycolumn decides which direction you want the counting to go in 
matrix2
...
matrix1[4, ] #
matrix1[ , 3] #this is to display rows and columns 
matrix1[c(12, 14)] #this is to call a specific element by slot number/position
matrix1[c(12:14)] #
matrix1[2:4, 1:3] #
...
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) #call function names to rename the columns for the three variables. 
...
> Answer: the type shows up in the environment tab. matrices and data frames show up as data. vectors and other single value variables show up as values. functions show up as a third category in the environment tab. 
...
low_F <- c(55, 56, 58, 60, 61, 63, 63, 64, 64, 63)
...
df_temp <- data.frame(high_F, low_F, high_C, low_C) #
names(df_temp) <- c("High (F)","Low (F)","High (C)", "Low (C)"); View(df_temp) #call function names to rename the columns for the four variables. 
...
summary(df_temp)
sd(df_temp$`High (F)`) #you cannot get the sd for an entire df, must pick a column!

```
# you can use rm() function in the console to remove something from your environment without sweeping the whole thing
### Date objects
...
```{r DateExercise2}
#century issue. there is cutoff in lubridate package that will think you're in the wrong century.
str_past <- "55-feb-3"
