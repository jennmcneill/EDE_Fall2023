...
the_registrant <- webpage %>% 
  #take the specific "cell" from the webpage that you're looking for 
  #here it's the "registrant"
  html_nodes(".table tr:nth-child(1) td:nth-child(2)") %>% 
  #and pass it through a pipe to get the text associated with that node
  html_text()
the_registrant
```
...
avg_withdrawals <- webpage %>% 
  #actually allows us to specify ranges of values
  html_nodes('.table:nth-child(7) td:nth-child(7) , .table:nth-child(7) td:nth-child(3)') %>%
  html_text()
#be careful and take note that numeric values are pulled as characters from the website
avg_withdrawals
...
#we didn't scrape the months, so we use the rep() command to make a vector of 1-12
#we can do the same thing for year using rep() and make a vector of 12 repeats of 2000
#convert average and max withdrawals to numeric with as.numeric() command
...
#!! aka "bang bang" tells it to refer to the thing as a variable not the string
...
#we build the scrape URL up by components and then concatenate them together
the_base_url <- 'https://www.ncwater.org/WUDC/app/WWATR/report/view'
the_facility <- '0218-0238'
the_year <- 2014
#paste0() eliminates adding a space between the elements that we
...
#repeat the function scrape.it for every value in the array the_years
#specify the second argument the_facility to the function
