...
huc8_sf_UpperNeuse <- st_read(here('Data/Spatial/NCHUC8.shp')) %>%
  filter(SUBBASIN == "Upper Neuse")


#Create a map
#if you replace the + with a vertical line (|), something else happens
mapView(huc8_sf, col.region = "orange") +
  mapView(huc8_sf_UpperNeuse, col.region = "purple")

#mapView(list(huc8_sf, huc8_sf_UpperNeuse), col.regions = list(orange, purple))

#ggplot() +
#geom_sf(data=huc8_sf, fill = 'orange') +
#geom_sf(data=huc8_sf_UpperNeuse, fill = 'purple')
...
the_filter = st_intersects(x = counties_sf_utm, y = neuse_sf, sparse=FALSE)
