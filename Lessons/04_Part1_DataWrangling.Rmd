...
`==` #compare your value to a specific value
`!=` #check if variables are different
`<` #check for values that are less than a specific value
`<=` #check for values that are less than or equal to a specific value
`>` #check for values that are greater than a specific value
`>=` #check for values that are greater than or equal to a specific value
`&` #aggregate multiple conditions to check simultaneously if all should be true
`|` #aggregate multiple conditions to check simultaneously if not all should be true
...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] #filter only the rows that have 0 for column depth, no rule for the columns)

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0) #first argument data set, second argument is what you're trying to filter
...
# filtering only chooses rows we are not doing anything with columns yet
...
#exclude the other lakes that you are not interested in, keeping peter and paul left
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))
#%in% helps you select more at once because you use names within the vector data set
...
NTL.phys.data.JunethruOctober4 <- filter(NTL.phys.data, daynum %in% c(152:304)) #think of this as a value matching operator
...
```{r using lubridate}
# add a month column to the dataset. returns the month for each observation.
...
# find out the start and end dates of the dataset using interval function using matrix positions or first and last
...
NTL.phys.data.processed <- 
  ...
  #create another column for temp in F
  mutate(temperature_F = (temperature_C*9/5) + 32)
#because this is all performed in a pipe you do not have to specify the original data set at each step
