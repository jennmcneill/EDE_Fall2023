...
Rainfall_BR_processed <-
  ...
  #similar to the text to column function in excel
  #make three different columns, one with all spaces, one with month, one with average. 
  #we are using the space as the separator. 
...
#when you call the summary, you can see that there are no NAs. 
#this isn't the best way to check for missing data. if there were full years missing,
#they would not show up as NAs
#it is better to create a sequence of dates in a dataframe
#you can do a left join of the new sequence on the original dataframe
#left join will populate NA
#then you do the summary
...
#the gray bars are how much you have to zoom in. bigger bar, less of an impact that has.
#the rate of change of the first 15 years of temperature is slower than the last 85
```
