...
## you can pull out a specific column using the $ command
```{r}
cars$speed
mtcars$cyl
```
...
head(df,2) #show the first 2 rows or any other number you want

#If you know the number of the column you want you can refer to that to access column
df[,1]
df[1:10,2] #column 2 for everything rows 1 through 10
df[c(1,2,3),2] #vector for column 2 rows 1 2 and 3. c() means combine. you could choose a diff order if you wanted
df[c(2,3,1),2] #different order shown here
