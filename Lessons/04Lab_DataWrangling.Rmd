...
# dplyr filtering
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0) #use equivalence operator ==

# use levels() or unique() to get unique values in a column
...
NTL.phys.data.TuesdayLake_pipe <-
  NTL.phys.data %>%
...
#NTL.phys.data.TuesdayLake_pipe_other <-
...
NTL.phys.data.TuesdayLake_pipe_2 <-
  NTL.phys.data %>%
  filter(lakename == 'Tuesday Lake') %>%
  mutate(month = month(sampledate)) %>%
  filter(month == 7) %>%
