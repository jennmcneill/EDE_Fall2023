...
#aes stands for aesthetics. this tells what data we want to plot and then specifying the sub data set. then it gives us the relative proportion of each. vast majority are approved (A). looking at the counts of each value in a category and plotting the categories with corresponding bars that are relative to the amount of values in that category.
ggplot(USGS.flow.data, aes(x = discharge.mean.approval)) +
  geom_bar()
#new plot omitting all of the NA values. we only have approved now.
...
```{r, fig.height = 3, fig.width = 4}
#not a normal distribution. skewed towards 0.
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean))

#supply the binwidth so that it doesn't use the default (30)
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

#you generally want to keep between 5 and 20 bins. if you do too many, it might start showing more patterns in the distribution than are truly necessary.
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

#highlight little subsets. this forces x to go just from 0 to 500. we can focus on where the majority of the data are. 
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
#this shows that there is a little peak that is higher than 0. you could change bin number to figure out more about the data.
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
# you can add multiple plots on top of each other using + sign. lty is line type.
ggplot(USGS.flow.data) +
...
#look at the value but color them based on the mean approval
ggplot(USGS.flow.data) +
...
#at low values of gage height, there are tighter values.
ggplot(USGS.flow.data) +
