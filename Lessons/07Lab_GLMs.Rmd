...
Ozone.density.plot <- ggplot(EPAair, aes(x = Ozone,
                                         fill = factor(Year))) +
  geom_density(alpha=0.5)+
  scale_fill_manual(values=c("tomato","yellow"))+
  xlab("Ozone AQI Value")+
  ylab("Density")
  # find a way to change the name of the legend with a similar command fill("Year")
  #geom_vline(xintercept = 50, color = "#238b45", lty = 2, size = 0.9) +
  #scale_x_continuous(expand = c(0, 0)) + 
  #scale_y_continuous(expand = c(0, 0))
print(Ozone.density.plot)

Ozone.box.plot <- ggplot(EPAair, aes(x = factor(Year),
                                     y = Ozone)) +
  geom_boxplot() +
  ...
  ylim(0,150)
print(Ozone.box.plot)

Ozone.violin.plot <- ggplot(EPAair, aes(x = factor(Year),
                                        y = Ozone)) +
  geom_violin(draw_quantiles = 0.5) +
  stat_summary(geom = "text",
               fun = max, #marks the max value with the label; you could also say min here
               vjust = -1,
               size = 4,
               label = c("b", "a")) +
  xlab("") +
